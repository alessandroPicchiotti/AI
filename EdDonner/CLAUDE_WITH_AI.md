...
1. Use your  tools to read the feature instructions from fordel root if exist
...
The backend should be in backend/ and be a uv project, using .NET 10. no minimal api .
The frontend should be in frontend/   Blazor web assembly
The database should use SQL Server and be created from scratch each time the Docker container is brought up, allowing for a users table with sign up and sign in.  
Consider statically building the frontend and serving it via .NET 10 no minimal api, if that will work.  
...
### Completed (PL-4)
- Docker multi-stage build (Node frontend + :ENT 10 backend)
- .NET 10 backend with SQL Server (fresh DB each container start)
- .NET 10 static export served by .NET 10 at localhost:8000
